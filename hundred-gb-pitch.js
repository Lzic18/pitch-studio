...
section.innerHTML='<div class="hundred-gb-heading"><div><h1 id="hundred-gb-title">100GB for £10 pitch</h1><p>Choose a pitch and copy it straight into your chat.</p></div><span class="hundred-gb-count">2 pitches</span></div><p id="hundred-gb-copy-status" class="hundred-gb-status" role="status" aria-live="polite"></p><div class="hundred-gb-grid"></div>';
...
replies.forEach(([title,text],i)=>{
 ...
 const body=document.createElement('p');body.className='hundred-gb-reply-text';const emphasis=i===0?'100GB of data for just £10 a month':'100GB of data for only £10 a month';
 const parts=text.split(emphasis);const strong=document.createElement('strong');strong.textContent=emphasis;body.append(parts[0],strong,parts[1]);
